      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end">
        <div className="grid flex-1 grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={m.label} className="metric-card relative overflow-hidden rounded-2xl px-4 py-4">
              <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${m.accent}`} />
              <p className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{m.label}</p>
              <p className={`mt-2 text-3xl font-black tracking-tight ${m.color}`}>{m.value}</p>
            </div>
          ))}
        </div>
        ...
        <button
          ...
          className="primary-action w-full shrink-0 sm:w-auto xl:mb-0.5"
        >
